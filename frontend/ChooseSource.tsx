...
export function ChooseSource({ appState, setAppState }) {
  ...
  const [labelField, setLabelField] = useState<Field>(null);

  const isValid = table && imageField && labelField;
    ...
    <Box display="flex" alignItems="center" justifyContent="center" border="default" flexDirection="column" width={viewport.size.width} height={viewport.size.height} padding={0}>
      <Box maxWidth='580px'>
        ...
        <form onSubmit={next}>
          ...
          {table &&
            <Box>
              <FormField label="Select Label Field (Only Single Select)">
                <FieldPicker
                  table={table}
                  field={labelField}
                  onChange={newField => setLabelField(newField)}
